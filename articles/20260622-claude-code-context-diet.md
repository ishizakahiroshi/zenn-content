...
この記事で使った「コンテキスト棚卸し → トリガー化分割 → settings.json チューニング」の手順を、他の人が自分の環境に適用できるように簡易の md スキルとして配布します。リポジトリ側に SKILL.md と説明、図解付きの詳細版 HTML を置く予定です。

- 配布リポジトリ: `github.com/ishizakahiroshi/claude-code-context-diet`（記事公開時点ではまだ用意中。準備でき次第このリンクが有効になります）
- 詳細版・図解版（github.io ホスト）: `ishizakahiroshi.github.io/articles/2026-06-22-claude-code-context-diet/`（同上）
